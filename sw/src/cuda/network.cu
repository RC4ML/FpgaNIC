		...
		*(ctx->listen_start)	=	0;
		...
		if(res==-1){//timeout
			...
			if(is_zero((unsigned int *)&res,0)){//todo
				printf("socket listen failed with reg:%x\n",res);
				return;
			}else{
...
__global__ void accept(socket_context_t* ctx,int* socket,connection_t* connection){
	...
	BEGIN_SINGLE_THREAD_DO
		...
		if(ctx->is_listening == 0 || socket_id!=ctx->server_socket_id){
			printf("accept failed, not listening!\n");
			return;
		}else if(ctx->is_accepting==1){
			printf("accept failed,is accepting!\n");
			return;
		}else{
			ctx->accepted=0;
	...
	if(ctx->buffer_used == MAX_BUFFER_NUM){
		...
		for(int i=0;i<MAX_BUFFER_NUM;i++){
			if(ctx->buffer_valid[i] == 0){
				printf("get empty buffer %d\n",i);
				return i;
			}
		}
		printf("error,buffer runs out, buffer_used is wrong!\n");
		return -1;
	}
}